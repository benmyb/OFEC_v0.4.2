...
public:
	...
	double getConstraintValue(vector<double> &val);
private:
	#ifdef OFEC_CONSOLE
	#if defined __GNUG__ && __GNUG__ >= 3
...
ReturnFlag Solution<ED>::evaluate(const bool rFlag){
	...
	#ifdef OFEC_DEMON 
	if (rf== Return_Invalid)	this->m_obj=m_worstSoFar->data().m_obj;
	#endif
	#ifdef OFEC_CONSOLE
		if(rFlag){
...
template<typename ED>
double Solution<ED>::getConstraintValue(vector<double> &val) {
	return Global::msp_global->mp_problem->getConstraintValue(*this, val);
}
#endif
