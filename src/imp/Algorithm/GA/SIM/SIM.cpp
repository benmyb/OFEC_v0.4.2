...
SIM::SIM(ParamMap &v) : GAPopulation<CodeVInt, GAIndividual<CodeVInt>>(v[param_popSize], true), m_num(0), \
m_saveFre(1500), m_selectK(5), m_PC(0.8), m_PM(0.2)
{
...
void SIM::selection(GAIndividual<CodeVInt> &indivl1, GAIndividual<CodeVInt> &indivl2)
{
...
GAIndividual<CodeVInt> SIM::crossover(const GAIndividual<CodeVInt> &indivl1, const GAIndividual<CodeVInt> &indivl2)
{
	if (indivl1 == indivl2)
		return indivl1;
...
void inverse_(int g, int gl, GAIndividual<CodeVInt> &indivl)
{
	int i, j, pos1, pos2;
	int pos;
...
void SIM::mutation(GAIndividual<CodeVInt> &indivl)
{
	int starts, ends;
	starts = Global::msp_global->getRandInt(0, m_numDim - 1);
	starts = indivl.data().m_x[(starts - 1 + m_numDim) % m_numDim];
	do
	{
		ends = Global::msp_global->getRandInt(0, m_numDim - 1);
		ends = indivl.data().m_x[ends];
	} while (ends == starts);
	inverse_(starts, ends, indivl);
}

bool SIM::ifTerminating()
{
