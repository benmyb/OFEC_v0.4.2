...
#ifndef MELOPERATOR_QAP_H
#define MELOPERATOR_QAP_H

#include "MELOperator.h"

template<typename TypeIndi>
class MELOperator_QAP :public MELOperator<TypeIndi> {
public:
	MELOperator_QAP(int numDim);
	~MELOperator_QAP() {}
...
template<typename TypeIndi>
MELOperator_QAP<TypeIndi>::MELOperator_QAP(int numDim) {
...
template<typename TypeIndi>
void MELOperator_QAP<TypeIndi>::updatePro( vector<unique_ptr<TypeIndi>> &pop, const vector<double>& weight, const vector<int> *index) {
...
template<typename TypeIndi>
void MELOperator_QAP<TypeIndi>::updatePro( vector<TypeIndi> &pop, const vector<double>& weight, const vector<int> *index) {
	int numDim = mvv_pro.size();
	int popsize = pop.size();
	for (int i = 0; i < numDim; ++i)
		for (int j = 0; j < numDim; ++j)
...
/*template<typename TypeIndi>
void MELOperator_QAP<TypeIndi>::updatePro(const vector<vector<int>> &pop, const vector<double>& weight) {
	int numDim = mvv_pro.size();
	for (int i = 0; i < numDim; ++i)
		for (int j = 0; j < numDim; ++j)
			mvv_pro[i][j] = 0;
...
*/
template<typename TypeIndi>
double MELOperator_QAP<TypeIndi>::findMinWeight(const vector<double>& weight, const vector<int> *index) {
	double minWeight;
	if (index == nullptr) {
		minWeight = weight[0];
...
template<typename TypeIndi>
void MELOperator_QAP<TypeIndi>::modifiyPro(const vector<double>& weight, const vector<int> *index) {
	int numDim = mvv_pro.size();
...
template<typename TypeIndi>
void MELOperator_QAP<TypeIndi>::createSolutions(vector<unique_ptr<TypeIndi>> &pop, vector<TypeIndi> &curPop, double alpha) {
	int popsize = pop.size();
	int numDim = mvv_pro.size();
...
template<typename TypeIndi>
ReturnFlag MELOperator_QAP<TypeIndi>::updateSolutions(vector<unique_ptr<TypeIndi>> &pop, vector<TypeIndi> &curPop, int& impRadio, int ms) {
