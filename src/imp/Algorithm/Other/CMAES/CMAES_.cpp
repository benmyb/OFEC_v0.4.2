...
CMAES::CMAES(ParamMap &v):Algorithm(-1, "CMAES"), m_pop(v[param_popSize])
{
	 ...
	 initialsFilePathName+= "Algorithm/Other/CMAES/initials.par";
	 signalsFilePathName+= "Algorithm/Other/CMAES/signals.par";
}
...
ReturnFlag CMAES::run_(){
	...
	while (!ifTerminating())/*!cmaes_TestForTermination(&evo)*/
	{
		...
		for (i = 0; i < cmaes_Get(&evo, "popsize"); ++i) {
			...
			*/
			int count = 0;
			while (!Global::msp_global->mp_problem->isValid(m_pop[i].data())&&++count<cmaes_Get(&evo, "popsize")) {
			//while (!Global::msp_global->mp_problem->isValid(m_pop[i].data())) {
				cmaes_ReSampleSingle(&evo, i);
				Global::msp_global->mp_problem->increaseEvalsByOne();
				if (ifTerminating()) 		break;
			}	
			
			if (count < cmaes_Get(&evo, "popsize")) {
				copy(pop[i], m_pop[i].data());				
			}
			if (ifTerminating()) {
				if(FFreePeak_OnePeak*p=dynamic_cast<FFreePeak_OnePeak*>(Global::msp_global->mp_problem.get())) p->outPeakRate();
				break;
			}
		}
	...
	free(xfinal);
	//cout << Global::msp_global->m_runId << endl;
