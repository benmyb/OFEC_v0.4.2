        ...
        if (*MaxPopulationSize > 1) {
            ...
            if (!HasFitness(Cost)) {
                if (*PopulationSize < *MaxPopulationSize) {
                    ...
                    ReplaceIndividualWithTour(*PopulationSize - 1, Cost,this);
                    if (TraceLevel >= 1)
                        PrintPopulation(this);
                }
            }
        ...
        do {
            ...
            if (N->FixedTo1 && N->FixedTo1 != N->FixedTo1Saved) {
                if (N->Id > N->FixedTo1->Id)
                    N->C[N->FixedTo1->Id] = (this->*Distance)(N, N->FixedTo1);
                else
                    N->FixedTo1->C[N->Id] = (this->*Distance)(N, N->FixedTo1);
            }
            if (N->FixedTo2 && N->FixedTo2 != N->FixedTo2Saved) {
                if (N->Id > N->FixedTo2->Id)
                    N->C[N->FixedTo2->Id] = (this->*Distance)(N, N->FixedTo2);
                else
                    N->FixedTo2->C[N->Id] = (this->*Distance)(N, N->FixedTo2);
            }
        }
        while ((N = N->Suc) != FirstNode);
    }
