...
static char *GetFileName(char *Line,LKH::LKHAlg *Alg);
static char *ReadYesOrNo(int *V,LKH::LKHAlg *Alg);
#undef max
static size_t max(size_t a, size_t b);

void LKH::LKHAlg::ReadParameters()
{
	...
	ostringstream os;
	os<<Global::g_arg[param_workingDir]<<"Problem/Combination/TSP/data/"<<ParameterFileName<<".par";
    if (ParameterFileName) {
     /*   if (!(ParameterFile = fopen(ParameterFileName, "r")))
            eprintf("Cannot open PARAMETER_FILE: \"%s\"",
                  ParameterFileName); */
		if(!(ParameterFile = fopen(os.str().c_str(), "r")))
		{
		//	printff("use default parameter sets !\n");
			return;
		}
    //    printff("PARAMETER_FILE = %s\n", ParameterFileName);
        ...
        while (1) {
            ...
            if (!(ParameterFileName = GetFileName(ReadLine(stdin),this))) {
                do {
                    printff("PROBLEM_FILE = ");
                    ProblemFileName = GetFileName(ReadLine(stdin),this);
    ...
    while ((Line = ReadLine(ParameterFile))) {
        ...
        if (!strcmp(Keyword, "ASCENT_CANDIDATES")) {
            ...
            if (!(InitialTourFileName = GetFileName(0,this)))
            ...
            if (!(InputTourFileName = GetFileName(0,this)))
            ...
            if (!(OutputTourFileName = GetFileName(0,this)))
            ...
            if (!(PiFileName = GetFileName(0,this)))
            ...
            if ((Token = gStrtok_r(0, Delimiters,&savePtr))) {
                ...
                while ((Token = gStrtok_r(0, Delimiters,&savePtr))) {
                    for (i = 0; i < strlen(Token); i++)
                        Token[i] = (char) toupper(Token[i]);
            ...
            if (!ReadYesOrNo(&SubsequentPatching,this))
            ...
            if (!(TourFileName = GetFileName(0,this)))
        ...
        if ((Token = gStrtok_r(0, Delimiters,&savePtr)) && Token[0] != '#')
