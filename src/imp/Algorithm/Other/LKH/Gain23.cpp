...
GainType LKH::LKHAlg::Gain23()
{	
	if(!OldReversed.get())
		OldReversed.reset(new short(0));
    ...
    if (!s1.get() || s1->Subproblem != FirstNode->Subproblem)
		s1.reset(FirstNode);
        ...
        do {
            ...
            for (Ns2 = s2->CandidateSet; (s3 = Ns2->To); Ns2++) {
                ...
                for (X4 = 1; X4 <= 2; X4++) {
                    ...
                    if (X4 == 1 && s4 != s1.get() && !Forbidden(s4, s1.get()) &&
                        2 * SegmentSize(s2, s3) <= Dimension &&
                        (!c || G2 - (this->*c)(s4, s1.get()) > 0) &&
                        (G3 = G2 - (this->*C)(s4, s1.get())) > 0 &&
                        (Gain = BridgeGain(s1.get(), s2, s3, s4, 0, 0, 0, 0, 0,
                                           G3)) > 0)
                        return Gain;
                    if (X4 == 2 &&
                        !Forbidden(s4, s1.get()) &&
                        (!c || G2 - (this->*c)(s4, s1.get()) > 0) &&
                        (Gain = G2 - (this->*C)(s4, s1.get())) > 0) {
                        Swap1(s1.get(), s2, s3);
                        return Gain;
                    }
                    if (G2 - s4->Cost <= 0)
                    ...
                    for (Ns4 = s4->CandidateSet; (s5 = Ns4->To); Ns4++) {
                        ...
                        for (X6 = 1; X6 <= 2; X6++) {
                            ...
                            G4 = G3 + (this->*C)(s5, s6);
                            Gain6 = 0;
                            if (!Forbidden(s6, s1.get()) &&
                                (!c || G4 - (this->*c)(s6, s1.get()) > 0) &&
                                (Gain6 = G4 - (this->*C)(s6, s1.get())) > 0) {
                                if (Case6 <= 2 || Case6 == 5 || Case6 == 6) {
                                    Make3OptMove(s1.get(), s2, s3, s4, s5, s6,
                                                 Case6);
                                    return Gain6;
                                }
                                if ((Gain =
                                     BridgeGain(s1.get(), s2, s3, s4, s5, s6, 0,
                                                0, Case6, Gain6)) > 0)
                                    return Gain;
                            }
                            Breadth6 = 0;
                            ...
                            for (Ns6 = s6->CandidateSet; (s7 = Ns6->To);
                                ...
                                for (X8 = 1; X8 <= 2; X8++) {
                                    ...
                                    if (FixedOrCommon(s7, s8)
                                        ...
                                        continue;
                                    G6 = G5 + (this->*C)(s7, s8);
                                    if ((!c || G6 - (this->*c)(s8, s1.get()) > 0) &&
                                        (Gain = G6 - (this->*C)(s8, s1.get())) > 0) {
                                        if (Case8 <= 15) {
                                            Make4OptMove(s1.get(), s2, s3, s4,
                                                         s5, s6, s7, s8,
                                                         Case8);
                                            return Gain;
                                        }
