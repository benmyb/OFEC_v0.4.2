...
static void alloc_memory(int n)
{
	...
	p_array.reset(new point[n]);
	n_free_e.reset(new int(3 * n));
	e_array.reset(new edge[*n_free_e.get()]);
	e=e_array.get();
	free_list_e.reset(new edge *[*n_free_e.get()]);
    for (i = 0; i < *n_free_e.get(); i++, e++)
        free_list_e.get()[i] = e;
}
...
{
	assert(*n_free_e > 0);
    return (free_list_e.get()[--*n_free_e]);
}

static void free_edge(edge * e)
{
    free_list_e.get()[(*n_free_e)++] = e;
}
