...
#endif
////***  converted from the sorce code of MOEA/D provided in the paper below
/// H. Li and Q. Zhang,  Multiobjective Optimization Problems with Complicated Pareto Sets,  
/// MOEA/D and NSGA-II, IEEE Trans on Evolutionary Computation, vol. 12,  no 2,  pp 284-302, April/2009
///*********************************************************************
...
protected:
	...
	void init_uniformweight_(int &sum, int numObj, vector<int> &namda, int &n);
...
void MOEAD<TypeIndiv,TypePop>::init_uniformweight()
{
	...
	else
	{
		...
		vector<int > namda;
		int sum=0;
		if (numObj == 4) m_unit = 17;
		else if(numObj == 5) m_unit = 11;
		init_uniformweight_(sum, numObj, namda,n);
		this->m_popsize=n;
	}
}

template<typename TypeIndiv, typename TypePop>
void MOEAD<TypeIndiv, TypePop>::init_uniformweight_(int &sum, int numObj,vector<int> &namda, int &n)
{
	if (numObj == 1) {
		if (sum <= m_unit) {
			namda.push_back(m_unit - sum);
			sum = m_unit;
			mv_namda.push_back(vector<double>(0));
			for (int k = 0; k < namda.size(); k++)
				mv_namda[n].push_back(1.0*namda[k] / m_unit);
			++n;
		}
	...
	for (int j = 0; j <= m_unit; ++j) {
		namda.push_back(j);	
		sum += j;
		init_uniformweight_(sum, numObj - 1, namda,n);
		
		while (namda.size()>Global::msp_global->mp_problem->getNumObj()- numObj) {		
			sum -= namda.back();
			namda.erase(namda.begin() + namda.size() - 1);			
		}
