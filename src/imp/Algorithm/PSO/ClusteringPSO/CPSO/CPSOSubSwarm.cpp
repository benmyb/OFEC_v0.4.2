...
ReturnFlag CPSOSubSwarm::evolve(){
	...
	if(r_flag==Return_Normal){
		...
		double ratio = 0;
		if(CAST_PROBLEM_DYN)Global::msp_global->mp_problem->getEvaluations() % CAST_PROBLEM_DYN->getChangeFre() / static_cast<double>(CAST_PROBLEM_DYN->getChangeFre());
		else if(CAST_PROBLEM_DYN_ONEPEAK) Global::msp_global->mp_problem->getEvaluations() % CAST_PROBLEM_DYN_ONEPEAK->getChangeInteval() / static_cast<double>(CAST_PROBLEM_DYN_ONEPEAK->getChangeInteval());
