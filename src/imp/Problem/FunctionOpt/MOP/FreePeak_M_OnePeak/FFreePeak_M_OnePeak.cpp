...
bool FFreePeak_M_OnePeak::isValid(const VirtualEncoding  &s){
	if (m_name == "FUN_FreePeak_M_C_OnePeak"){
...
void FFreePeak_M_OnePeak::createPeaks_(){
	...
	if (Global::g_arg.find(param_dataFile3) != Global::g_arg.end() && Global::g_arg.find(param_dataFile1) != Global::g_arg.end()){
		...
		for (int num = 0; num < m_numBox*m_numObj; ++num) {
			string s;
			getline(in, s);
			vector<string> col(gGetStringNumItems(s));
			stringstream ss(s);
			...
			peak[pidx].transf.resize(FOnePeak::VariableTransfm::NR_ITEMS_VT);
			for (auto&j : peak[pidx].transf) j = 0;
			peak[pidx].transf[0] = 1;
			for (auto &j : transf) {
				if (j < FOnePeak::DistanceTransfm::NR_ITEMS_DT - 1) peak[pidx].transf[0] = j + 1;
				else peak[pidx].transf[j - FOnePeak::DistanceTransfm::NR_ITEMS_DT + 2] = 1;
			}

			gGetStringValue(col[5], value);
			if (value[0].is_int())	peak[pidx].noisy = (int)value[0];
			else peak[pidx].noisy = value[0];

			for (int i = 0; i < FOnePeak::Constraint::NR_ITEMS_C; ++i) {
				gGetStringValue(col[i + 6], value);
				vector<double> par(value.size());
				pair<bool, vector<double>> con;
				con.first = (int)value[1];
				for (int j = 2; j < value.size(); ++j) {
					if (value[0].is_int())	con.second.push_back((int)value[j]);
					else con.second.push_back(value[j]);
				}
				peak[pidx].constraint.push_back(con);
			}
			...
			for (int i = 0; i < m_tree[t]->region.size(); ++i, ++bidx){
				...
				for (int j = 0; j < m_peaksPerBox; ++j){
					...
					if (m_case == Jump || m_case == Web) {
						...
						p = new FOnePeak(Global::msm_pro[name], m_numDim, name, peak[m_numPeak].loc, m_objShape[m_numPeak], peak[m_numPeak].hei);
						if ((m_objShape[m_numPeak] == FOnePeak::SH7 || m_objShape[m_numPeak] == FOnePeak::SH8 || m_objShape[m_numPeak] == FOnePeak::SH9 || m_objShape[m_numPeak] == FOnePeak::SH10)) {
							p->setRadiusStepFun(peak[m_numPeak].radius);
						}
					}					
					p->gidx() = i;
