...
void FFreePeak_OnePeak::createPeaks_(){
	...
	if (Global::g_arg.find(param_dataFile1) != Global::g_arg.end()){
		...
		iss.clear();
		iss.str(s);
		vector<TypeVar> value_;
		gGetStringValue(s, value_);
		m_numGOpt=value_.size();
		getline(in, s);
		...
		while (!in.eof()){
			...
			peak[bidx].transf.resize(FOnePeak::VariableTransfm::NR_ITEMS_VT);
			for (auto&j : peak[bidx].transf) j = 0;
			peak[bidx].transf[0] = 1;
			for (auto &j : transf){
				if (j < FOnePeak::DistanceTransfm::NR_ITEMS_DT-1) peak[bidx].transf[0] = j + 1;
				else peak[bidx].transf[j - FOnePeak::DistanceTransfm::NR_ITEMS_DT +2] = 1;
			}
			
			gGetStringValue(col[5], value);
			if (value[0].is_int())	peak[bidx].noisy = (int)value[0];
			else peak[bidx].noisy = value[0];
			...
			for (int i = 0; i < FOnePeak::Constraint::NR_ITEMS_C; ++i) {
				...
				peak[bidx].constraint.push_back(con);
			}
		}
		m_numPeak = m_numBox;
		int idxp = 0, idxb = 0;
		for (int t = 0; t < m_tree.size(); ++t){
			for (int i = 0; i < m_tree[t]->region.size(); ++i, ++idxb){
				...
				for (int j = 0; j < m_peaksPerBox; ++j){
					FOnePeak *p = nullptr;
					p = new FOnePeak(Global::msm_pro[name], m_numDim, name, peak[idxb].loc, peak[idxb].shape, peak[idxb].hei, &peak[idxb].transf, idxp,&peak[idxb].constraint,peak[idxp].noisy);
					p->gidx() = i;
					p->idx() = j;
					p->tidx() = t;
