...
public:
	...
	static void generateDivision(const vector<vector<pair<double, double>>> &box, vector<int> &node, int numNode, int numDim, int numTree, const string &path, bool evendiv = false, vector<double> *br = 0, int fileid = -1);
	static void generateLocationSingleObj(int numDim, int numPeak, const string & path, bool random = true, int fileid = -1);  // mode=true denotes that peak locations are randomly generated in the space, otherwise at the center
	static vector<int> generateLocationMultiObj(int numDim, int numPeak, int numObj, float minRadius /*[0,1]*/, const string & path, int mode, int numParetoRegion = 1, float rhoweb = 1., int fileid = -1); //mode=1: Jump, mode=2: Web 
	static void generateConf(const vector<Peak> &conf, const vector<int> &gbox, const string & path, int obj = 1, int fileid = -1);
	static void generateConfGOP(int numPeak, const string & path, int gShape, vector<int> &gTrans, string & gPosition, bool gNoise, vector<pair<bool, vector<double>>>& gCons,
		int lShape, vector<int> & lTrans, double lMaxH, bool lRandH, bool lNoise, vector<pair<bool, vector<double>>>& lCons,
		bool trap, double tMinH, double tMaxH, int fileid = -1);
	static void generateConfMMOP(int numPeak, const string & path, vector<int> &gShape, vector<int> &gTrans, const vector<string> & gPosition, bool gNoise, vector<pair<bool, vector<double>>>& gCons, int gNum,
		int lShape, vector<int> & lTrans, double lMaxH, bool lRandH, bool lNoise, vector<pair<bool, vector<double>>>& lCons,
		bool trap, int tNum, double tMinH, double tMaxH, vector<string> & tPosition, int fileid = -1);
	static void generateConfMOP(int numBox, const string & path, vector<int> &oShape, int numObj, int mode, float rhoHeight, vector<int>*gboxweb = 0, int numParetoRegion = 1, int fileid = -1); //mode=1: Jump, mode=2: Web, mode=3: Countable
